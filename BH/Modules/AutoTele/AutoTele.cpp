...
void AutoTele::OnLoad() {
	...
	settingsTab = new UITab("ÔÓÏî", BH::settingsUI);
	unsigned int Y = 12;
	unsigned int col = 40;

	new Texthook(settingsTab, col+20, Y, "¿ª¹Ø");

	new Checkhook(settingsTab, col, (Y += 15),
			&Toggles["Draw Destination"].state, "Òýµ¼Ïß");

	new Checkhook(settingsTab, col, (Y += 15),
			&Toggles["Quest Drop Warning"].state, "ÈÎÎñÎïÆ·µôÂäÌáÐÑ");

	new Texthook(settingsTab, col+20, (Y += 22), "´´½¨ÓÎÏ·");
	new Checkhook(settingsTab, col, (Y += 15),
			&(*bnetBools)["Autofill Last Game"], "×Ô¶¯Ìî³äÓÎÏ·Ãû");
	new Checkhook(settingsTab, col, (Y += 15),
			&(*bnetBools)["Autofill Next Game"], "ÏÂÒ»¸öÓÎÏ·+1");
	new Checkhook(settingsTab, col, (Y += 15),
			&(*bnetBools)["Autofill Last Password"], "×Ô¶¯Ìî³äÓÎÏ·ÃÜÂë");
	new Checkhook(settingsTab, col, (Y += 15),
			&(*bnetBools)["Autofill Description"], "×Ô¶¯Ìî³äÓÎÏ·ËµÃ÷");
	new Checkhook(settingsTab, col, (Y += 15),
			&(*gamefilterBools)["Show Difficulty"], "ÏÔÊ¾ÄÑ¶È");
	new Checkhook(settingsTab, col, (Y += 15),
			&(*gamefilterBools)["Show Gameserver"], "ÏÔÊ¾·þÎñÆ÷");
	...
	new Colorhook(settingsTab, 250, 42, &Colors[1], "ÏÂÒ»³¡¾­");

	new Colorhook(settingsTab, 250, 57, &Colors[2], "Èë¿Ú");

	new Colorhook(settingsTab, 250, 72, &Colors[3], "Ð¡Õ¾");

	new Colorhook(settingsTab, 250, 87, &Colors[4], "ÉÏÒ»³¡¾°");

	new Colorhook(settingsTab, 250, 102, &Colors[5], "ÆäËûÈë¿Ú");
}
